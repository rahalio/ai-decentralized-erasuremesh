...
import { DOMAIN_PREFIX_MAP, isValidDomainId } from '@erasuremesh/core';
import { ulid } from 'ulid';
import type { IdGeneratorService } from '@erasuremesh/services/_shared';
...
export class DefaultIdGeneratorService implements IdGeneratorService {
  ...
  pgrId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.pgr);
  }
  eroId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.ero);
  }
  darId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.dar);
  }
  rcnId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.rcn);
  }
  evpId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.evp);
  }
  excId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.exc);
  }
