 ...
 *
 * DDD: Infrastructure adapter implementing EvidencePack repository using DynamoDB.
 * Generated from x-dynamodb metadata in OpenAPI schema.
 */

import type { EvidencePackRepository } from "@erasuremesh/services/evidence-packs";
...
/**
 * EvidencePackRepositoryDdb using DynamoDB table (type: core)
 *
 * Entity Type: EVIDENCE_PACK
 * Entity Token: EVIDENCE_PACK
 * SK Prefix: EVIDENCE_PACK
 */
export class EvidencePackRepositoryDdb implements EvidencePackRepository {
  ...
  private readonly ENTITY_TYPE = "EVIDENCE_PACK";
  private readonly ENTITY_TOKEN = "EVIDENCE_PACK";
  private readonly SK_PREFIX = "EVIDENCE_PACK";
  ...
  private readonly DOMAIN = "EVIDENCE_PACKS";
  private readonly ENTITY_TYPE_PK = "EVIDENCE_PACKS";
  private generateCorrelationId(): string {
    const domainPrefix = "evp";
  ...
  async getEvidencePack(input: Parameters<EvidencePackRepository['getEvidencePack']>[0]): Promise<Awaited<ReturnType<EvidencePackRepository['getEvidencePack']>>> {
    ...
    if (!(inputExt.id ?? inputExt.packId)) {
      throw new Error("Missing required parameter: id");
    }
    if (!inputExt.orgId) {
      throw new Error("Missing required parameter: orgId");
    }
    const pk = this.buildPK(inputExt.orgId as string, (inputExt.id ?? inputExt.packId) as string);
    // Soft-delete layout prefixes SK with ACTIVE# when statusPrefix is enabled.
    const skPrefix = `${this.ENTITY_TOKEN}#${(inputExt.id ?? inputExt.packId)}#`;
    ...
    if (!item) {
      throw new Error(`${this.ENTITY_TYPE} with id ${(inputExt.id ?? inputExt.packId)} not found`);
    }
    return {
  ...
  async assembleEvidencePack(input: Parameters<EvidencePackRepository['assembleEvidencePack']>[0]): Promise<Awaited<ReturnType<EvidencePackRepository['assembleEvidencePack']>>> {
    const inputExt = input as Record<string, unknown>;
